...
type Service struct {
	...
	lookupCache  sync.Map // 缓存模型名到 pricing entry 的映射
}
...
func (s *Service) getPricing(model string) (*PricingEntry, bool) {
	...
	// 先检查缓存
	if cached, ok := s.lookupCache.Load(model); ok {
		if cached == nil {
			return nil, false
		}
		return cached.(*PricingEntry), true
	}

	// 精确匹配
	if entry, ok := s.pricingMap[model]; ok {
		s.lookupCache.Store(model, entry)
		return entry, true
	}

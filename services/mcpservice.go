...
type MCPServer struct {
	...
	EnablePlatform      []string          `json:"enable_platform"` // "claude-code", "codex", "gemini-cli"
...
// loadGeminiEnabledServers loads enabled MCP servers from Gemini CLI config
...
// syncGeminiServers syncs MCP servers to Gemini CLI config
func (ms *MCPService) syncGeminiServers(servers []MCPServer) error {
	...
	desired := make(map[string]map[string]any)
	for _, server := range servers {
		if !platformContains(server.EnablePlatform, platGeminiCLI) {
			continue
		}
...
// buildGeminiEntry builds a Gemini CLI MCP server entry
...
// geminiConfigPath returns the path to Gemini CLI MCP config

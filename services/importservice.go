...
// --- Config Export/Import via Deep Linking (Phase 5.4) ---

// ExportOptions specifies what to include in the export
type ExportOptions struct {
	IncludeProviders bool     `json:"include_providers"`
	IncludeMCP       bool     `json:"include_mcp"`
	Platforms        []string `json:"platforms"` // ["claude", "codex", "gemini-cli"]
	FilterAPIKeys    bool     `json:"filter_api_keys"`
}

// ExportConfig is the data structure for exported configuration
...
// ExportConfig exports current configuration as Base64-encoded JSON
func (is *ImportService) ExportConfig(options ExportOptions) (string, error) {
	config := ExportConfig{
		Version: "1.0",
	}
	...
	if options.IncludeProviders {
		...
		if len(platforms) == 0 {
			platforms = []string{"claude", "codex", "gemini-cli"}
		}

		for _, platform := range platforms {
			providers, err := is.providerService.LoadProviders(platform)
			if err != nil {
				continue // Skip platforms that fail to load
			}

			// Filter API keys if requested
	...
	if options.IncludeMCP {
		servers, err := is.mcpService.ListServers()
		if err == nil {
...
// ImportFromBase64 imports configuration from Base64-encoded JSON string
func (is *ImportService) ImportFromBase64(encodedConfig string) (ConfigImportResult, error) {
	// Decode Base64
	jsonData, err := base64.URLEncoding.DecodeString(encodedConfig)
	if err != nil {
		// Try standard encoding if URL encoding fails
		jsonData, err = base64.StdEncoding.DecodeString(encodedConfig)
		if err != nil {
			return ConfigImportResult{}, fmt.Errorf("invalid base64 encoding: %w", err)
		}
	}
	...
	if err := json.Unmarshal(jsonData, &config); err != nil {
		return ConfigImportResult{}, fmt.Errorf("invalid config format: %w", err)
	}

	// Validate version
	if config.Version != "1.0" {
		return ConfigImportResult{}, fmt.Errorf("unsupported config version: %s", config.Version)
	}

	result := ConfigImportResult{
		Status: ConfigImportStatus{
			ConfigExists: true,
			ConfigPath:   "deep-link",
		},
	}
	...
	if len(config.Providers) > 0 {
		...
		providersByPlatform := make(map[string][]Provider)
		for _, p := range config.Providers {
			// Infer platform from provider (assume we have a way to determine this)
			// For now, we'll try to save to all platforms
			providersByPlatform["claude"] = append(providersByPlatform["claude"], p)
		}

		for platform, providers := range providersByPlatform {
			...
			// Save merged providers
			if newProviders > 0 {
				if err := is.providerService.SaveProviders(platform, existing); err == nil {
					result.ImportedProviders += newProviders
				}
			}
		}
	}
	...
	if len(config.MCPServers) > 0 {
		...
		// Save merged servers
		if newServers > 0 {
			if err := is.mcpService.SaveServers(existing); err == nil {
				result.ImportedMCP = newServers
			}
		}
	}
...
// ParseDeepLink parses a codeswitch:// URL and extracts the config parameter
func (is *ImportService) ParseDeepLink(deepLink string) (string, error) {
	// Parse URL
	...
	// Check scheme
	if u.Scheme != "codeswitch" {
		return "", fmt.Errorf("invalid URL scheme: expected 'codeswitch', got '%s'", u.Scheme)
	}

	// Check host (action)
	if u.Host != "import" {
		return "", fmt.Errorf("unsupported action: %s", u.Host)
	}

	// Extract config parameter
	query := u.Query()
	encodedConfig := query.Get("config")
	if encodedConfig == "" {
...
// ImportFromDeepLink imports configuration from a codeswitch:// deep link
func (is *ImportService) ImportFromDeepLink(deepLink string) (ConfigImportResult, error) {
	// Parse deep link to extract config
	encodedConfig, err := is.ParseDeepLink(deepLink)
	if err != nil {
		return ConfigImportResult{}, err
	}

	// Import from Base64
	return is.ImportFromBase64(encodedConfig)
}
